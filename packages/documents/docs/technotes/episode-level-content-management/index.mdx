...
of the player, actually developers can pass their own 
...
* **Naïve components:** If there's no need to let **Episode Core** controlling
the lifecycle of the component, this component is a naïve component, we don't
need to do anything special, it will works fine.

* **Managed components:** There can be other kinds of components. The most notable
kind is the transition animation components that can temporarily block the
content switching process. See "Switching between contents" section below.
...
* **Asset Instance Component:** The user interface side of every ContentInstance
is also a component. For more information, see "Content and ContentInstance"
section below.
