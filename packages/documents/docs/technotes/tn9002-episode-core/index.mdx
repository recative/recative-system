...
an audio file. There's an extension can handle this task in _Recative Studio_,
...
- `SubsequenceManager`: Allows the assset to create and control
...
- `destroyed`: The `ContentInstance` is completely destroyed, allthe resources
  it used are released.
...
assets, time calculation and Synchronition.
...
Assets can create their own subsequences. `subsequence`s are similar to the main
sequence, but some behavior are managed by the parent sequence:

- The subsequence will be hidden if its parent content is hidden.
- The subsequence won't play if its parent content is not playing.
