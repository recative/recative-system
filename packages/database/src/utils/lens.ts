...
export const isDotNotation = (field: ValidLensField) => {
...
type DotNotationImplementation<T, K extends keyof T> =
  K extends string
    ...
    ? T[K] extends Record<string, any>
      ...
      ? T[K] extends ArrayLike<any>
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ? K | `${K}.${DotNotationImplementation<T[K], Exclude<keyof T[K], keyof any[]>>}`
...
type DotNotationValue<T, P extends DotNotation<T>> =
...
type PathValue<T, K extends string[] | Readonly<string[]>> =
  K extends [infer O] | Readonly<[infer O]>
    ? O extends keyof T
      ? T[O]
      : never
...
/**
  ...
  * getIn({a: {b: 1}}, "a.b", true) => 1
  */
export const lens = <T, P extends ValidLensField>(
  object: T,
  path: P,
  usingDotNotation = isDotNotation(path),
): LensResult<T, P> | undefined => {
  ...
  if (!usingDotNotation) {
    if (typeof path !== 'string') {
      throw new TypeError('The path should be a string if not using not notation option');
    }
    return Reflect.get(object, path);
  }
  ...
  if (
    ...
    throw new TypeError('Unable to use dot notation with number or symbol path');
  }
  ...
  const internalPath = typeof path === 'string'
    ? path.split('.')
